...
VALUES	('TRAIT_CIVILIZATION_PATCHERESU_GUNSHIP',	'KIND_TRAIT'	),
		...
		('ABILITY_PATCHERESU_GUNSHIP',				'KIND_ABILITY'	);

-----------------------------------------------
-- Tags

-- Certain game functions are aligned to groups of units (known as classes), rather than the individual unit-types themselves. For this reason, here we define the class (CLASS_PATCHERESU_GUNSHIP) - we will later associate the unit to this class (as the only unit of that class) but also the UnitAbility to that same class. The below table INSERT paves the way for the special ability to be applied to our custom unit.
-----------------------------------------------	
	
INSERT INTO Tags
		(Tag,						Vocabulary		)
VALUES	('CLASS_PATCHERESU_GUNSHIP',		'ABILITY_CLASS'	);

-----------------------------------------------
-- TypeTags

-- As mentioned above, we now link the Unit itself (UNIT_PATCHERESU_GUNSHIP) to our newly-defined class (CLASS_PATCHERESU_GUNSHIP). The second set of values associate the newly-defined Unit Ability (ABILITY_PATCHERESU_GUNSHIP) to all objects within that same class (CLASS_PATCHERESU_GUNSHIP).

-- The second INSERT function ensures our new Unit (UNIT_PATCHERESU_GUNSHIP) inherits the classes that the UNIT_SPEARMAN already has configured. This ensures that groups to which the UNIT_SPEARMAN belongs, the UNIT_PATCHERESU_GUNSHIP also belongs. This ensures integration with other Unit Abilities and identifications and allows for a cohesive replacement.
-----------------------------------------------		
...
VALUES	('TRAIT_CIVILIZATION_PATCHERESU_GUNSHIP',	'LOC_UNIT_PATCHERESU_GUNSHIP_NAME',		'LOC_UNIT_PATCHERESU_GUNSHIP_DESCRIPTION'	);
...
VALUES	('CIVILIZATION_PATCHERESU_GARLEMALD',		'TRAIT_CIVILIZATION_PATCHERESU_GUNSHIP'		);
...
-- You will note that the full set of values is populated via a hybrid of methods - some are taken from the UNIT_SPEARMAN that UNIT_PATCHERESU_GUNSHIP replaces, whilst others are brand-new values that we've defined in this mod.

-- For integer values, it is possible to set a relative value by using simple mathematical operators - you'll note that we do this for the unit Cost and the Combat (Strength). What we're saying here is that the UNIT_PATCHERESU_GUNSHIP has a Cost that is 15 higher than the UNIT_SPEARMAN, but it also has a Combat Strength of 5 more.
...
-- This section defines the position in the UnitUpgrades chain that the custom UNIT_PATCHERESU_GUNSHIP fits into. For this mod, we are simply replacing the UNIT_SPEARMAN, so this ensures the continuity of the UNIT_SPEARMAN chain for the UNIT_PATCHERESU_GUNSHIP.
-----------------------------------------------
...
SELECT 	'UNIT_PATCHERESU_GUNSHIP',	UpgradeUnit
FROM 	UnitUpgrades
...
-- In the below example, we are simply inheriting the UNIT_SPEARMAN behaviour for our UNIT_PATCHERESU_GUNSHIP.
...
SELECT 	'UNIT_PATCHERESU_GUNSHIP',		AiType
FROM 	UnitAiInfos
WHERE 	UnitType = 'UNIT_BIPLANE';
...
-- This command tells the game about the replacement itself - this ensures that for any civilization to whom the UNIT_PATCHERESU_GUNSHIP is available (which is only this custom civilization, thanks to the link to the TraitType), the UNIT_SPEARMAN will not be available; replaced instead by the UNIT_PATCHERESU_GUNSHIP.
-----------------------------------------------
...
-- We are using this kind of requirement due to the nature of the ability. We are giving a particular unit a combat boost during a Golden Age. Golden Ages are granted to a civilization and the civilization is the owner of the unit in question. In this case, context is important. The use of OwnerRequirementSetId ensures it is only when the custom civilization, that has the UNIT_PATCHERESU_GUNSHIP, is in a Golden Age will the effect apply. SubjectRequirementSetId would be used in cases where we want to check a variable is true in the context of the unit - for example, that it is in a certain terrain, or that it has a particular type of opponent in combat.
...
-- This simply ties the two elements together - it ensures that any unit that has the ABILITY_PATCHERESU_GUNSHIP UnitAbilityType will receive the bonus granted by the MODIFIER_GUNSHIP_GOLDEN_AGE_STRENGTH Modifier.
